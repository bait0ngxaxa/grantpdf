...
export async function POST(request: NextRequest) {
    try {
        const { projectName, clientName, description, userId } =
            await request.json();
        ...
        const formPdfBytes = await fetch(templateUrl).then((res) => {
            if (!res.ok)
                throw new Error(
                    `Failed to fetch PDF template from ${templateUrl}: ${res.statusText}`
                );
        ...
        const fontBytes = await fetch(fontUrl).then((res) => {
            if (!res.ok)
                throw new Error(
                    `Failed to fetch font from ${fontUrl}: ${res.statusText}`
                );
            return res.arrayBuffer();
        });
        ...
        const sanitizedProjectName = projectName
            ...
            .replace(/[<>:"/\\|?*]/g, ""); // ลบเฉพาะอักขระที่ไม่อนุญาตในชื่อไฟล์
        ...
        try {
            const userFile = await prisma.userFile.create({
                data: {
                    originalFileName: `${projectName}.pdf`,
