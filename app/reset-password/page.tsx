...
export default function ResetPasswordPage() {
    ...
    // and expiry immediately, but for this example, we'll rely on the backend API.
    if (!token && !message) {
    ...
    const handleResetPassword = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                setMessage(data.message || "รีเซ็ตรหัสผ่านสำเร็จ! กรุณาเข้าสู่ระบบด้วยรหัสผ่านใหม่");
            } else {
                ...
                <div className="card w-full max-w-lg bg-white dark:bg-gray-800 shadow-2xl rounded-2xl transform transition-transform duration-300 hover:scale-[1.01] overflow-hidden">
                    <div className="card-body p-8">
                        <div className="flex flex-col items-center mb-6">
                            <h2 className="text-3xl font-bold text-center">ตั้งรหัสผ่านใหม่</h2>
                            ...
                            <form onSubmit={handleResetPassword} className="space-y-6">
                                ...
                                <div className="form-control">
                                    <label className="label">
                                        <span className="label-text font-medium text-gray-600 dark:text-gray-300">รหัสผ่านใหม่</span>
                                    </label>
                                    <input
                                        type="password"
                                ...
                                <div className="form-control">
                                    <label className="label">
                                        <span className="label-text font-medium text-gray-600 dark:text-gray-300">ยืนยันรหัสผ่านใหม่</span>
                                    </label>
                                    <input
                                        type="password"
                                        className="input input-bordered w-full rounded-full transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
