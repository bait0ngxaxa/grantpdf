...
export const ProjectItem: React.FC<ProjectItemProps> = ({
    ...
    return (
        <div
            className={cn(
                "group bg-white dark:bg-slate-800 rounded-3xl shadow-sm border transition-[border-color,box-shadow] duration-300 overflow-hidden",
                isExpanded
        ...
        >
            ...
            >
                <div className="flex items-start space-x-4 min-w-0">
                    <div
                        className={cn(
                            "w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex-shrink-0 flex items-center justify-center transition-colors duration-300",
                            isExpanded
                            ...
                            <div className="sm:hidden relative">
                                {hasUnreadStatusNote && (
                                    <span className="absolute -top-1 -right-1 flex h-2.5 w-2.5">
                                        <span className="animate-ping motion-reduce:animate-none absolute inline-flex h-full w-full rounded-full bg-orange-400 opacity-75" />
                                ...
                                <button
                                    ...
                                    className={cn(
                                        "group/status inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-bold shadow-sm cursor-pointer transition-[transform,box-shadow,background-color,color,border-color] duration-200 hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-1",
                                        statusClassName,
                                    )}
                                ...
                                >
                                    ...
                                    {project.status ||
                                        PROJECT_STATUS.IN_PROGRESS}
                                    <ChevronRight className="h-3 w-3 opacity-80 transition-transform duration-200 group-hover/status:translate-x-0.5" />
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex items-center justify-between sm:justify-end gap-3 pl-16 sm:pl-0">
                    ...
                    <div className="hidden sm:block relative">
                        ...
                        <button
                            onClick={onStatusClick}
                            className={cn(
                                "group/status inline-flex items-center gap-2 rounded-full border px-3.5 py-1.5 text-xs font-bold shadow-sm cursor-pointer transition-[transform,box-shadow,background-color,color,border-color] duration-200 hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-1",
                                statusClassName,
                            )}
                            title="คลิกเพื่อดูรายละเอียดสถานะ"
                            aria-label={`ดูรายละเอียดสถานะโครงการ: ${
                        ...
                        >
                            ...
                            {project.status || PROJECT_STATUS.IN_PROGRESS}
                            <span className="hidden lg:inline text-[10px] font-semibold opacity-75">
                                ดูรายละเอียด
                            </span>
                    ...
                    <div
                        className={cn(
                            "p-2 rounded-full flex-shrink-0 transition-transform duration-300",
                            isExpanded
